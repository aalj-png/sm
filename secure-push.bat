@echo off
REM 🚀 SECURE GITHUB PUSH - Token passed as parameter
REM Usage: secure-push.bat "your-token-here"
REM Or: secure-push.bat (will prompt for token)
...
REM Get token from parameter or prompt
if "%~1"=="" (
...
REM Build the repository URL with token (only in memory, not in files)
set GITHUB_URL=https://%GITHUB_TOKEN%@github.com/%GITHUB_USERNAME%/%GITHUB_REPO%.git
...
echo.
echo ===============================================================
echo  ^^ SECURE GITHUB PUSH WITH TOKEN ^^
echo ===============================================================
echo.
